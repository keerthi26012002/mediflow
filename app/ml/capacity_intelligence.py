...
def calculate_capacity_metrics(state: Dict[str, Any], recent_events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Computes 25 advanced Hospital Capacity Intelligence metrics based on
    the authoritative Digital Twin state and recent events.
    """
    # 1. Utilizations
    ...
    # 2. Inflow / Outflow & Turnover
    # Query admissions & discharges count in recent events
    ...
    # Turnover rate: admissions + discharges in window
    ...
    recent_oxygen_vals = [e.get("oxygen_utilization") for e in recent_events if "oxygen_utilization" in e]
    if len(recent_oxygen_vals) >= 2:
        resource_burn_rate = float(recent_oxygen_vals[0] - recent_oxygen_vals[-1])
    else:
    ...
    # Capacity Trend: rate of change of capacity score
    recent_loads = [e.get("hospital_load_index") for e in recent_events if "hospital_load_index" in e]
    if len(recent_loads) >= 2:
        capacity_trend = float(recent_loads[-1] - recent_loads[0]) # positive = load increasing -> capacity decreasing
    else:
        capacity_trend = 0.0
